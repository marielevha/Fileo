...
export async function listPublished(kind:ContentKind,limit=100):Promise<ContentRow[]>{return sql<ContentRow>(`select ${contentSelect} from public.contents where ${logicalKind}=$1 and status='published' and locale='fr' order by sort_order,published_at desc limit $2`,[kind,limit]);}
