...
import { Archive, ChevronDown, ChevronLeft, ChevronRight, ChevronUp, Plus, RotateCcw, Search, UsersRound } from 'lucide-react-native';
...
export function ClientsScreen() {
  ...
  const load = useCallback(async (refresh = false) => {
    ...
    setError(null);
    try {
      const next = await listClients({ page, pageSize: 10, q: debouncedQuery, includeArchived, sort, direction }, (cached) => {
      ...
      if (requestId.current === currentRequest) setError(caught instanceof Error ? caught.message : 'Impossible de charger les clients.');
    } finally {
    ...
    <SafeAreaView edges={['top']} style={[styles.safeArea, { backgroundColor: theme.colors.background }]}>
      <FlatList
        ...
        ListHeaderComponent={<View>
          ...
          <View style={styles.resultLine}><AppText color={theme.colors.textMuted} variant="caption">{searchPending ? 'Recherche' : result ? `${result.total} client${result.total > 1 ? 's' : ''}` : 'Chargement'}</AppText></View>
        ...
        ListEmptyComponent={loading || searchPending ? <ActivityIndicator color={theme.colors.primary} size="large" style={styles.loader} /> : error || !result ? null : <View style={styles.empty}>
          <UsersRound color={theme.colors.textSubtle} size={36} />
          <AppText style={styles.emptyTitle} variant="title3">{hasSearch ? 'Aucun client trouvé' : 'Votre carnet est vide'}</AppText>
          <AppText color={theme.colors.textMuted} style={styles.emptyCopy}>{hasSearch ? 'Aucun nom ou numéro ne correspond à cette recherche.' : 'Ajoutez votre premier client pour retrouver ses commandes et mensurations ici.'}</AppText>
          <Pressable accessibilityRole="button" onPress={hasSearch ? () => setQuery('') : () => router.push('/atelier/clients/nouveau')} style={emptyStyles.action}>
            {hasSearch ? <RotateCcw color={palette.white} size={18} /> : <Plus color={palette.white} size={18} />}
            <AppText color={palette.white} variant="label">{hasSearch ? 'Effacer la recherche' : 'Créer un client'}</AppText>
          </Pressable>
        </View>}
...
const emptyStyles = StyleSheet.create({
  action: { alignItems: 'center', backgroundColor: palette.violet700, borderRadius: 8, flexDirection: 'row', gap: 8, justifyContent: 'center', marginTop: 20, minHeight: 44, paddingHorizontal: 18 },
});
